...
bool   AddSpheres::use_mpi = false;
bool   AddSpheres::logarithmic = false;

AddSpheres::AddSpheres(std::shared_ptr<AxiSymModel> halo,
		       std::shared_ptr<AxiSymModel> bulge, double Bmass)
{
  ...
  for (int i=0; i<number; i++) {
    ...
    m[i] = halo->get_mass(r[i]) + bmass*bulge->get_mass(r[i]);
    dm[i] = bmass*bulge->get_mass(r[i]);
  }
 ...
 mod = std::make_shared<SphericalModelTable>
...
#ifdef DEBUG
  {
    ostringstream outf;
    if (use_mpi)
      outf << "test_add_mod." << myid << '\0';
    else
      outf << "test_add_mod.dat\0";
    ofstream out(outf.str().c_str());
    for (int i=1; i<number; i++) {
      out 
