...
double Disk2dHalo::RHMAX       = 50.0;
double Disk2dHalo::RDMIN       = 1.0e-4;
double Disk2dHalo::RDMAX       = 20.0;
double Disk2dHalo::Q           = 0.0;
double Disk2dHalo::SIG0        = 0.1;
double Disk2dHalo::XI          = 1.0;
...
int    Disk2dHalo::NHR         = 800;
int    Disk2dHalo::NHT         = 40;
int    Disk2dHalo::SEED        = 11;
int    Disk2dHalo::ITMAX       = 1000000;

double Disk2dHalo::RA          = 1.0e20;
...
int    Disk2dHalo::NCHEB       = 8;
bool   Disk2dHalo::CHEBY       = false;
bool   Disk2dHalo::ALLOW       = false;
bool   Disk2dHalo::use_mono    = true;

unsigned Disk2dHalo::VFLAG     = 7;
unsigned Disk2dHalo::NBUF      = 65568;

string Disk2dHalo::RUNTAG      = "debug";

// lower-case string to enum map for setting disk-velocity type
std::map<std::string, Disk2dHalo::DiskGenType>
Disk2dHalo::getDiskGenType = {
  {"jeans",      Disk2dHalo::Jeans     },
  {"asymmetric", Disk2dHalo::Asymmetric},
  {"epicyclic",  Disk2dHalo::Epicyclic }
};
	   ...
	   DiskGenType type)
{
  ...
  if (myid==0 && VFLAG & 1) {
    std::cerr << "Disk2dHalo: DIVERGE=" << DIVERGE
	 << " A=" << A
	 << " RDMAX=" << RDMAX
	 << " filename=" << filename
	 << std::endl;
...
Disk2dHalo::~Disk2dHalo()
{
  // Nothing
}

Disk2dHalo::Disk2dHalo(const Disk2dHalo &p)
{
  newmod = p.newmod;
  halo   = p.halo;
  halo2  = p.halo2;
...
void Disk2dHalo::set_halo(vector<Particle>& phalo, int nhalo, int npart)
{
  if (!MULTI) {
    string msg("Disk2dHalo::set_halo is only valid if MULTI is true");
    throw Disk2dHaloException(msg, __FILE__, __LINE__);
  }
...
void Disk2dHalo::disk_eval(double R, double z, double phi,
			 double &p, double &fr, double &fz, double &fp)
{
  double d0, d1, p0, p1;
  std::tie(d0, d1, p0, p1, fr, fz, fp) = expandd->accumulated_eval(R, z, phi);
  p = p0 + p1;
}
...
table_disk(vector<Particle>& part)
{
  ...
  if (true and myid==0) {
	...
	tout << std::setw(16) << nrD[i]
      ...
      std::cout << "Disk2dHalo: could not open test file <mass.debug>"
		<< std::endl;
    }
  }
  ...
  if (Q <= 0.0) {
    ...
    if (SIG0 <= 0.0)
      throw std::runtime_error("Disk2dHalo: radial dispersion must be positive.");

    if (XI <= 0.0)
      throw std::runtime_error("Disk2dHalo: dispersion axis ratio must be positive.");
...
//  
double Disk2dHalo::vp_disp2(double xp, double yp, double zp)
{
...
// Constant Toomre Q
//
double Disk2dHalo::vr_disp2(double xp, double yp, double zp)
{
  double r = sqrt(xp*xp + yp*yp);
...
//
double Disk2dHalo::a_drift(double xp, double yp, double zp)
{
				// sigma_r and sigma_p
  ...
  if (1) {
	...
	cp[0]>1.0 || cp[0] < 0.0 )
      std::cerr << "Disk2dHalo::a_drift: phi=" << phi
	   << ", cp=(" << cp[0] << ", " << cp[1] << ")" << std::endl;
    if (cr[1]>1.0 || cr[1] < 0.0 ||
	cr[0]>1.0 || cr[0] < 0.0 )
      std::cerr << "Disk2dHalo::a_drift: R=" << R
	   << ", cr=(" << cr[0] << ", " << cr[1] << ")" << std::endl;
  }
  // END DEBUG
...
//
double Disk2dHalo::v_circ(double xp, double yp, double zp)
{
  double R = sqrt(xp*xp + yp*yp);
  ...
  if (vcirc2<=0.0) {
    if (VFLAG & 8)
      std::cout << "Disk2dHalo::v_circ: circular velocity out of bounds, R="
...
set_vel_disk(vector<Particle>& part)
{
  ...
  double vr, vp, R, x, y, z, ac, vc, va, as, ad;
  ...
  for (auto &p : part) {
	...
	out << std::setw(14) << R   << std::setw(14) << z   << std::setw(14) << vc
	    ...
	    << std::setw(14) << vr  << std::setw(14) << vp  
	    << std::setw(14) << vvR << std::setw(14) << vvP 
...
void Disk2dHalo::table_halo_disp()
{
  if (!MULTI) {
    string msg("Disk2dHalo::table_halo_disp is only valid if MULTI is true");
    throw Disk2dHaloException(msg, __FILE__, __LINE__);
  }
  ...
  if (rmin<=0.0 && LOGR) {
    string msg("Disk2dHalo::table_halo_disp: LOGR=1 but rmin<=0");
    throw Disk2dHaloException(msg, __FILE__, __LINE__);
  }
...
double Disk2dHalo::get_disp(double xp,double yp, double zp)
{
  if (MULTI) {
  ...
  if (halotable.cols() != NHR) {
    std::cerr << "Disk2dHalo::get_disp: must call table_halo first\n";
    MPI_Abort(MPI_COMM_WORLD, 100);
    exit(0);
  }
...
void Disk2dHalo::set_vel_halo(vector<Particle>& part)
{
  if (!expandh) {
...
void Disk2dHalo::write_record(ostream &out, SParticle &p)
{
...
void Disk2dHalo::flush_buffer(ostream &out)
{
...
void Disk2dHalo::write_file(ostream &fou, vector<Particle>& part)
{
...
void Disk2dHalo::virial_ratio(vector<Particle>& hpart, vector<Particle>& dpart)
{
...
void Disk2dHalo::virial_ratio(const char *hfile, const char *dfile)
{
  ifstream in[2];
...
void Disk2dHalo::profile(ostream &out, vector<Particle>& dpart,
...
void Disk2dHalo::disk_model(const std::string &modfile)
{
  std::vector<double> r2(RNUM);
