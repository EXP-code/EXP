...
def plot_data(filename, degree, smooth, msize, dens, tmax=-1.0, tscale=1.0e5, logY=False, dot=False, fullScreen=False):
...
def main(argv):
    ...
    option_string = '[-D <density> | --density=<density> | -d <degree> | --deg=<degree> | -s <smooth> | --smooth=<smooth> | -m <size> | --msize=<size> | -l | --log | -p | --point | -t tscale | --timescale=<tscale> |  -T tmax | --tmax=<tmax> | -F | --full-screen] <runtag>'

    try:
        opts, args = getopt.getopt(argv,'hD:d:s:m:t:T:lpF', ['density=', 'deg=', 'smooth=', 'msize=', 'timescale=', 'tmax=', 'log', 'point', 'full-screen'])
    except getopt.GetoptError:
        print(sys.argv[0], option_string)
        sys.exit(2)
    for opt, arg in opts:
        if opt == '-h':
            print(sys.argv[0], option_string)
    ...
    plot_data(filename, degree, smooth, msize, dens, tmax, tscale, logY, dot, fullS)
