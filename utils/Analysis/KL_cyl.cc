...
main(int argc, char **argv)
{
  ...
  ortho0.setHall("test", psp->GetNamed(cname)->comp.nbod);
  ...
  if (myid==0) std::cout << std::endl
			 ...
			 << std::flush;
				...
				// Number of particles
  unsigned nbod = psp->GetNamed(cname)->comp.nbod;
				// Size of bunch
  ...
  do {
	...
	for (int mm=0; mm<=mmax; mm++) {
      ...
      coefs.push_back(boost::make_shared<CoefStruct>(mmax, nmax));
  ...
  if (myid==0) cout << "Making coefficients for bunch ["
  ...
  OrthoCoefs ac_cos(mmax+1), ac_sin(mmax+1);
  for (int mm=0; mm<=mmax; mm++) {
    ac_cos[mm].setsize(0, nmax-1);
    ac_sin[mm].setsize(0, nmax-1);
  }
  ...
  for (int nsnr=0; nsnr<NSNR; nsnr++) {
	...
	for (int j=0; j<coefs.size(); j++) {
	  KL[j] += p->mass * log(fabs(DD[ibnch]/DD[j]));
	}
    ...
    // Synchronize all KL
    //
    MPI_Allreduce(MPI_IN_PLACE, KL.data(), coefs.size(),
		  MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
