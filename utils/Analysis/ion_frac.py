...
polynomical fitting with degree 2.  To use a spline fit, specify the value of the smoothing parameter, i.e.
	...
	$ python ion_frac -s 10000000 run2
...
def plot_data(filename, degree, smooth, msize, tscale=1.0e5, logY=False, dot=False, fullScreen=False):
...
def main(argv):
    ...
    option_string = '[-d <degree> | --deg=<degree> | -s <smooth> | --smooth=<smooth> | -m <size> | --msize=<size> | -l | --log | -p | --point | -F | --full-screen] <runtag>'

    try:
        opts, args = getopt.getopt(argv,'hd:s:m:t:lpF', ['deg=', 'smooth=', 'msize=', 'timescale=', 'log', 'point', 'full-screen'])
    ...
    plot_data(filename, degree, smooth, msize, tscale, logY, dot, fullS)
