...
def plot_data(filename, eloss, msz, logY, dot, tscale):
        ...
        if line.find('#')<0:        # Read the data lines
            ...
            if not allZ:            
                ...
                if len(toks) == len(labels):
                    ...
                    misE.append(float(toks[-4]))
                    clrE.append(float(toks[-5]))
...
def main(argv):
    ...
    parser.add_argument('-e', '--eloss',   default=False, action='store_true', help='Plot energy lost')
    ...
    plot_data(args.tag[0], args.eloss, args.size, args.log, args.point, args.tscale)
