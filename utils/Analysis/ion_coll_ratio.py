...
def plot_data(filename, msz, dot, summary, izr, smooth):
    ...
    if not summary:
        ...
        for k in range(0, 4):
            ...
            for v in spec:
                ...
                if izr:
                    ...
                    lws = sm.nonparametric.lowess(ratio, temp)
                    pl.semilogy(lws[:,0], lws[:,1], '-', label=v)
                else:
                    pl.semilogy(temp, ratio, mk, label=v, markersize=msz)
            pl.title(elabs[k])
            if icnt==4:
        ...
        for v in spec:
            ...
            for k in range(len(ekeys)):
                ...
                if izr:
                    ...
                    pl.semilogy(temp, ratio, mk, label=elabs[k], markersize=msz)
            pl.title(v)
            leg = pl.legend(loc='best',borderpad=0,labelspacing=0)
            leg.get_title().set_fontsize('6')
            pl.setp(pl.gca().get_legend().get_texts(), fontsize='12')
        ...
        if smooth:
            ...
            pl.semilogy(temp, ratio, mk, label='He+ ionize', markersize=msz)
        ...
        if smooth:
            ...
            pl.semilogy(temp, ratio, mk, label='He++ recomb', markersize=msz)
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(argv,"hm:pszL", ["help", "msize=", "point", "summary", "ignore", "lowess"])
    except getopt.GetoptError:
        print 'Syntax Error'
        print sys.argv[0], '[-p | --point | -m <size> | --msize=<size> | -s | --summary | -L | --lowess] <runtag>'
        sys.exit(2)
    for opt, arg in opts:
        if opt in ("-h", "--help"):
            print sys.argv[0], '[-p | --point | -m <size> | --msize=<size> | -s | --summary | -L | --lowess] <runtag>'
    ...
    plot_data(filename, msz, dot, rpt, izr, lss)
