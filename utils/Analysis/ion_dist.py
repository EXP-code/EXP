...
def plot_data(runtag, field, defaultT, until, deltaT):
    ...
    for line in file:
        ...
        if look:
            ...
            if result is not None:
                ...
                lz = []
                mid = len(ly)/2
                for v in xx: lz.append(ly[mid]-slope*(v-xx[mid])+0.5*np.log(v/xx[mid]))
                ...
                lz = []
                for v in xx: lz.append(ly[mid]-slope*(1.0+deltaT)*(v-xx[mid])+0.5*np.log(v/xx[mid]))
                ...
                ax.plot(xx, lz, '-')
                ax.set_title("{}: t={} T={}[+/-]{}".format(field,time,defaultT,deltaT*defaultT))
                ...
                zz = []
                for v in xx: zz.append(np.exp(ly[mid]-slope*(1.0+deltaT)*(v-xx[mid])+0.5*np.log(v/xx[mid])))
                ay.plot(xx, zz, '-.')
...
def main(argv):
    ...
    options = '[-f <field> | --field=<field> | -t <initial time> | --time=<initial time> | -T <temp> | --temp=<temp> | -d <delta> | --delta=<delta>] <runtag>'

    try:
        opts, args = getopt.getopt(argv,"hf:T:t:d:", ["help","field=","temp=","time=","delta="])
    ...
    plot_data(filename, field, defT, until, delta)
