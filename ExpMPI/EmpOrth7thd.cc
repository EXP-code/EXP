...
extern pthread_mutex_t used_lock, cos_coef_lock, sin_coef_lock;
...
EmpCylSL::~EmpCylSL(void)
{
  ...
  if (accum_cos2) delete [] accum_cos2;
  if (accum_sin2) delete [] accum_sin2;
...
int EmpCylSL::cache_grid(int readwrite)
{
  ...
  if (readwrite) {
    ...
    out.write(&NZOF, sizeof(int));
    out.write(&NUMR, sizeof(int));
    out.write(&NORDER, sizeof(int));
    out.write(&DENS, sizeof(int));
    ...
    in.read(&nzof, sizeof(int));
    in.read(&numr, sizeof(int));
    in.read(&norder, sizeof(int));
    in.read(&dens, sizeof(int));
...
void EmpCylSL::setup_accumulation(void)
{
  ...
  if (!accum_cos) {
    ...
    if (SELECT) {
      accum_cos2 = new Vector [MMAX+1];
      accum_sin2 = new Vector [MMAX+1];
    }
#ifdef DEBUG_NAN
  ...
  for (int m=0; m<=MMAX; m++) {
    ...
    accum_cos[m].zero();
    if (SELECT) {
      accum_cos2[m].setsize(0, rank3-1);
      accum_cos2[m].zero();
    }
    if (m>0) {
      ...
      accum_sin[m].zero();
      if (SELECT) {
	accum_sin2[m].setsize(0, rank3-1);
	accum_sin2[m].zero();
      }
    }
  }
...
void EmpCylSL::setup_eof()
{
  if (!SC) {
    ...
    for (int i=0; i<nthrds; i++) {
      vc[i].setsize(0, max(1,MMAX), 0, rank3-1);
      vs[i].setsize(0, max(1,MMAX), 0, rank3-1);
    }
...
{
  if (eof_made) {
    cerr.form("accumulate_eof: Process %d, Thread %d calling setup_eof()\n",
	      myid, id);
  ...
  for (mm=0; mm<=MMAX; mm++) {
    ...
    for (int ir=1; ir<=NUMR; ir++) {
      ...
      for (int i=1; i<=rank2; i++) {
	pthread_mutex_lock(&cos_coef_lock);
	accum_cos0[mm][ir][i] += facC[id][ir][i] * mass;
	pthread_mutex_unlock(&cos_coef_lock);
	if (mm) {
	  pthread_mutex_lock(&sin_coef_lock);
	  accum_sin0[mm][ir][i] += facS[id][ir][i] * mass;
	  pthread_mutex_unlock(&sin_coef_lock);
	}

	for (int j=i; j<=rank2; j++) {
	  pthread_mutex_lock(&cos_coef_lock);
	  SC[mm][ir][i][j] += facC[id][ir][i]*facC[id][ir][j] * mass;
	  pthread_mutex_unlock(&cos_coef_lock);
	  if (mm) {
	    pthread_mutex_lock(&sin_coef_lock);
	    SS[mm][ir][i][j] += facS[id][ir][i]*facS[id][ir][j] * mass;
	    pthread_mutex_unlock(&sin_coef_lock);
	  }
	}
      }
...
void EmpCylSL::make_eof(void)
{
  ...
  if (myid==0) {
	...
	MPI_Isend(&request_id, 1, MPI_INT, s, 1, MPI_COMM_WORLD, &req[0]);
...
void EmpCylSL::accumulate(double r, double z, double phi, double mass, int id)
{
  ...
  if (coefs_made) {
    cerr.form("EmpCylSL::accumulate: Process %d, Thread %d: calling setup_accumulation from accumulate\n", myid, id);
    setup_accumulation();
  }
  ...
  for (mm=0; mm<=MMAX; mm++) {
    ...
    hold = norm * mass * mcos * vc[id][mm];
    pthread_mutex_lock(&cos_coef_lock);
    accum_cos[mm] += hold;
    if (SELECT) accum_cos2[mm] += hold & hold/mass;
    pthread_mutex_unlock(&cos_coef_lock);
    if (mm>0) {
      hold = norm * mass * msin * vs[id][mm];
      pthread_mutex_lock(&sin_coef_lock);
      accum_sin[mm] += hold;
      if (SELECT) accum_sin2[mm] += hold & hold/mass;
      pthread_mutex_unlock(&sin_coef_lock);
    }
  }
...
void EmpCylSL::make_coefficients(void)
{
  ...
  if (SELECT) {
    for (mm=0; mm<=MMAX; mm++)
      for (nn=0; nn<rank3; nn++)
	MPIin[mm*rank3 + nn] = accum_cos2[mm][nn];
    ...
    for (mm=0; mm<=MMAX; mm++)
      for (nn=0; nn<rank3; nn++)
	accum_cos2[mm][nn] = MPIout[mm*rank3 + nn];
  }  
  ...
  if (SELECT) {
    for (mm=1; mm<=MMAX; mm++)
      for (nn=0; nn<rank3; nn++)
	MPIin[mm*rank3 + nn] = accum_sin2[mm][nn];
    ...
    for (mm=1; mm<=MMAX; mm++)
      for (nn=0; nn<rank3; nn++)
	accum_sin2[mm][nn] = MPIout[mm*rank3 + nn];
  }
...
#ifdef DEBUG_PCA
  cerr.form("Process %d: made it to pca_hall\n", myid);
#endif
...
#ifdef DEBUG_PCA
  cerr.form("Process %d: using %d particles\n", myid, cylused);
#endif

  for (mm=0; mm<=MMAX; mm++)
    for (nn=0; nn<rank3; nn++) {
      sqr = accum_cos[mm][nn]*accum_cos[mm][nn];
      //      fac = (accum_cos2[mm][nn] - sqr + 1.0e-10)/(sqr + 1.0e-18)/cylused;
      fac = (accum_cos2[mm][nn] - sqr + 1.0e-10)/(sqr + 1.0e-18);
#ifdef DEBUG_PCA
      if (myid==0) cerr.form("%3d, %3d, %2s:  %13.4e, %13.4e, %13.4e\n", 
			     mm, nn, "C", 
			     accum_cos2[mm][nn], sqr, fac);
#endif
      accum_cos[mm][nn] *= 1.0/(1.0 + fac);
    }
  

  for (mm=1; mm<=MMAX; mm++)
    for (nn=0; nn<rank3; nn++) {
      sqr = accum_sin[mm][nn]*accum_sin[mm][nn];
      //      fac = (accum_sin2[mm][nn] - sqr + 1.0e-10)/(sqr + 1.0e-18)/cylused;
      fac = (accum_sin2[mm][nn] - sqr + 1.0e-10)/(sqr + 1.0e-18);
#ifdef DEBUG_PCA
      if (myid==0) cerr.form("%3d, %3d, %2s:  %13.4e, %13.4e, %13.4e\n", 
			     mm, nn, "S", 
			     accum_sin2[mm][nn], sqr, fac);
#endif
      accum_sin[mm][nn] *= 1.0/(1.0 + fac);
    }
  
#ifdef DEBUG_PCA
  cerr.form("Process %d: exiting to pca_hall\n", myid);
#endif
}
...
void EmpCylSL::dump_coefs_binary_curr(FILE *fout, double time)
{
  ...
  for (int mm=0; mm<=MMAX; mm++) {
    ...
    for (int j=0; j<rank3; j++)
      fwrite(&accum_cos[mm][j], sizeof(double), 1, fout);

    if (mm) {

      for (int j=0; j<rank3; j++)
	fwrite(&accum_sin[mm][j], sizeof(double), 1, fout);
