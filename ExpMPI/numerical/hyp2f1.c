...
double fabs(), pow(), rint(), log(), exp();
double psi();
static double hyt2f1(), hys2f1();
...
s = 1.0 - x;
flag = 0;
ia = rint(a); /* nearest integer to a */
ib = rint(b);
...
if( c <= 0.0 )
	{
	ic = rint(c); 	/* nearest integer to c */
...
p = c - a;
ia = rint(p); /* nearest integer to c-a */
if( (ia <= 0.0) && (fabs(p-ia) < EPS) )	/* negative int c - a */
	flag |= 4;

r = c - b;
ib = rint(r); /* nearest integer to c-b */
if( (ib <= 0.0) && (fabs(r-ib) < EPS) )	/* negative int c - b */
	flag |= 8;

d = c - a - b;
id = rint(d); /* nearest integer to d */
q = fabs(d-id);
