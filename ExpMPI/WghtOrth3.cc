...
void CylindricalSL::read_coefs_binary(istream *in, double* time)
{
  *time = -999.0;		// Default error signal
  ...
  int tst = in->rdstate();
  in->read(&coefheader, sizeof(CoefHeader));
  tst = in->rdstate();
  ...
  for (int mm=0; mm<=MMAX; mm++) {
	...
	znorm = 0.5/M_PI/ZMAX;
      ...
      for (int j=1; j<=NMAX; j++)
	in->read(&accum_cos[mm][nk  ][j], sizeof(double));

      for (int j=1; j<=NMAX; j++)
	in->read(&accum_cos[mm][nk+n][j], sizeof(double));
	...
	for (int j=1; j<=NMAX; j++)
	  in->read(&accum_sin[mm][nk  ][j], sizeof(double));

	for (int j=1; j<=NMAX; j++)
	  in->read(&accum_sin[mm][nk+n][j], sizeof(double));
