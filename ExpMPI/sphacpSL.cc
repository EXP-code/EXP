 ...
 *  the spherical biorthogonal expansion
 *
 *
 *  Call sequence:
 *  -------------
  ...
  if (myid==1 && EJcyl) 
    {
      ...
      ofstream debugf(toutfile, ios::app);
      Vector axis = orient->currentAxis();
      debugf.form("%10.4f  %13.4e  %13.4e  %13.4e  %13.4e  %13.4e  %13.4e  %13.4e  %13.4e  %13.4e  %10.4f  %i\n",
		  tnow, 
		  ...
		  orient->currentE(), orient->currentUsed() );
    }
...
void dump_mzero(char* name, int step)
{
  ...
  for (int i=0; i<4; i++) {
    ...
    out[i]->write(&ncylnx, sizeof(int));
    out[i]->write(&ncylny, sizeof(int));
    out[i]->write(&(zz=  0.0), sizeof(float));
    out[i]->write(&(zz= RMAX), sizeof(float));
    out[i]->write(&(zz=-ZMAX), sizeof(float));
    out[i]->write(&(zz= ZMAX), sizeof(float));
  }
  ...
  for (int k=0; k<ncylny; k++) {
    ...
    for (int j=0; j<ncylnx; j++) {
      ...
      out[0]->write(&zz, sizeof(float));

      ortho->accumulated_eval(r, z, 0.0, p, fr, fz, fp);
      out[1]->write(&(zz=p ), sizeof(float));
      out[2]->write(&(zz=fr), sizeof(float));
      out[3]->write(&(zz=fz), sizeof(float));
    }
  }
