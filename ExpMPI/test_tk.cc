...
void get_potl_dens_CBDisk(int lmax, int nmax, double r, double **potd, 
			  double **dend);
...
static void zeroth_order(String name)
{
  ...
  for (int i=0; i<=NUM; i++) {
	...
	get_dens_coefs_bes(M_pca, flag.array(1, nmax), &d);
      else {
	cerr << "tk: no expansion defined" << endl;
	_exit(-3);
      }
...
extern "C" void out_pca(int k)
{
  ...
  for (int im=0; im<imult; im++) {
	...
	covar[n][nn] = fac*cc[indx][n][nn]*fac02;
	if (n!=nn)
	...
	* sqrt(normM[L_pca][nn]);
      ...
      fac = 1.0/(1.0 + smooth*var/(dd*dd*used));
      smth[n] = dd*fac;
  ...
  for (l=L0, loffset=0; l<=lmax; loffset+=(2*l+1), l++) {
	...
	if (compute) {
	  ...
	  for (n=1; n<=nmax; n++) {
		...
		weight[indx][n] = 1.0/(1.0 + tksmooth*var/(dd*dd*used));
	...
	inv = evec[indx]*smth;
	for (n=1; n<=nmax; n++) expcoef[indx][n] = inv[n]/sqnorm[lm][n];
