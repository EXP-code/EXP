...
void set_parm(char *word, char *valu)
{
  ...
  else if (!strcmp("orbtrace",word))
    orbtrace = atoi(valu);

  else if (!strcmp("norbid",word))
    norbid = atoi(valu);
  ...
  else if (!strcmp("orbtracefile",word))
    strcpy(orbtracefile, valu);
...
void print_parm(FILE *stream, char *comment)
{
  ...
  fprintf(stream,"%s%-15s = %d\n",comment,"ndiag",ndiag);
  fprintf(stream,"%s%-15s = %d\n",comment,"orbtrace",orbtrace);
  fprintf(stream,"%s%-15s = %d\n",comment,"norbid",norbid);
  ...
  fprintf(stream,"%s%-15s = %s\n",comment,"orbtracefile",orbtracefile);
}
