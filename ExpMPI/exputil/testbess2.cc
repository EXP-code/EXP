...
main(int argc, char **argv)
{
  ...
  for (int i=1; i<=m; i++)
    cout << setw(4) << i << ">  " << setw(18) << ans[i] 
      << setw(18) << jn(n, ans[i])*n/ans[i] - jn(n+1, ans[i]) 
      << setw(18) << 0.5*(jn(n, ans[i]+eps) - jn(n, ans[i]-eps))/eps
	<< endl;

  ofstream to("testbess.dat");

  double r, dr = 1.2*ans[m]/400;
  for (i=1; i<=400; i++) {
    r = dr*i;
    to << setw(18) << r << setw(18) << jn(n, r)
      << setw(18) << 0.5*(jn(n, r+eps) - jn(n, r-eps))/eps
	<< endl;
  }
