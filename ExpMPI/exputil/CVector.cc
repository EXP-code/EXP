...
void CVector::setsize(int l, int h)
{
	...
	if (h<l) bomb_CVector("invalid size <0");
...
void bomb_CVector(const char *msg)
{
  cerr << "CVECTOR ERROR: " << msg << endl;
  exit(0);
}

void bomb_CVector_operation(const char *op)
{
  string msg("incompatible lengths in CVector operation ");
  msg += op;
  bomb_CVector(msg.c_str());
}
...
void CMatrix::setsize(int rl, int rh, int cl, int ch)
{
	...
	if (rh<rl || ch<cl) bomb_CMatrix("invalid size<=0");
...
void bomb_CMatrix(const char *msg)
{
  cerr << "CMATRIX ERROR: " << msg << endl;
  exit(0);
}

void bomb_CMatrix_operation(const char *op)
{
