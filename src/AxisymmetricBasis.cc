...
AxisymmetricBasis:: AxisymmetricBasis(Component* c0, const YAML::Node& conf) :
  Basis(c0, conf) 
{
  // Remove matched keys
  //
...
{
  if (muse <= 0.0) return;
  if (dof==3) pca_hall_3d(compute);
  if (dof==2) pca_hall_2d(compute);
}

void AxisymmetricBasis::pca_hall_3d(bool compute)
{
...
void AxisymmetricBasis::pca_hall_2d(bool compute)
{
  if (muse <= 0.0) return;
  ...
  if (compute) {
    ...
    for (int l=L0, loffset=0; l<=Lmax; l++) {
	...
	for (int nn=0; nn<nmax; nn++) {
	  initVar[nn] = (*expcoef[indx])[nn] * (*expcoef[indx])[nn];
	  if (l) initVar[nn] += (*expcoef[indx+1])[nn] * (*expcoef[indx])[nn];
	  initVar[nn] = sqrt(initVar[nn]);
	}
...
#ifdef HAVE_VTK
	if (vtkpca and myid==0) {
	  vtkpca->Add(meanJK, s_Hall[indxC], snrval, evalJK, evecJK.transpose(), covrJK, l);
	}
#endif
      }
	...
	for (int n=0; n<nmax; n++) {
	  ...
	  if (pcavar) {
	    ...
	    double tv = (*expcoef[indx])[n] * (*expcoef[indx])[n];
	    if (l) tv += (*expcoef[indx+1])[n] * (*expcoef[indx+1])[n];
...
void AxisymmetricBasis::parallel_gather_coefficients(void)
{
  ...
  if (dof==2) {
    for (int l=L0, loffset=0; l<=Lmax; l++) {
      ...
      if (l==0) {
	MPI_Reduce(&(*expcoef1[loffset])[0], 
		   &(*expcoef [loffset])[0], nmax, 
		   MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
	loffset++;
      }
      else {
	...
	MPI_Reduce(&(*expcoef1[loffset+1])[0],
		   &(*expcoef [loffset+1])[0], nmax, 
		   MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
...
void AxisymmetricBasis::parallel_distribute_coefficients(void)
{
  ...
  if (dof==2) {
    ...
    for (int l=L0, loffset=0; l<=Lmax; l++) {
      ...
      if (l==0) {
	MPI_Bcast(&(*expcoef[loffset])[0], nmax, MPI_DOUBLE,
		  0, MPI_COMM_WORLD);
	loffset++;
      }
      else {
	MPI_Bcast(&(*expcoef[loffset])[0], nmax, MPI_DOUBLE,
		  0, MPI_COMM_WORLD);
	MPI_Bcast(&(*expcoef[loffset+1])[0], nmax, MPI_DOUBLE,
		  0, MPI_COMM_WORLD);
	loffset+=2;
      }
    }
