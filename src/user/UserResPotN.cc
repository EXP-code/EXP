...
UserResPotN::UserResPotN(const YAML::Node& conf) : ExternalForce(conf)
{
  LMAX = 2;
  NMAX = 20;
  NUMR = 800;
...
void UserResPotN::determine_acceleration_and_potential(void)
{
  ...
  if (first) {
	...
	if (system(command.c_str()) == -1) {
	  std::cerr << "UserResPotN: error in executing <"
		    << command << ">" << endl;
	}
...
extern "C" {
  ExternalForce *makerResPotN(const YAML::Node& conf)
  {
    return new UserResPotN(conf);
  }
}
