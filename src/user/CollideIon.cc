				...
				double *cr, int id)
{
  ...
  for (size_t i = 0; i < dCross[id].size(); i++) {
	...
	if (kEe1[id] > 10.2) { ctd2->eV_10[id] += cF; }
      }
	...
	if (ExactE and q < 1.0) {
	  ...
	  if (AlgOrth) {
	    ...
	    if (wnrm>1.0e-12*sqrt(vcm2)) {
	      ...
	      w1   = v1;
	      vrat = sqrt( (q*b1f2/v1i2 + 1.0)/(1.0 - q) );
	    }
