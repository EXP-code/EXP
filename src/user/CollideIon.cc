				...
				const std::string& msg)
{
  ...
  /*
  if (use_cons>=0) {
    KEi1 += p1->dattrib[use_cons];
    KEi2 += p2->dattrib[use_cons];
  }

  if (use_elec>=0) {
    KEe1 += p1->dattrib[use_elec+3];
    KEe2 += p2->dattrib[use_elec+3];
  }
  */
				...
				double *cr, const Interact::T& itype, double prob)
{
  ...
  // KE at the same time.
  //
  double et1i = 0.0, et2i = 0.0, iE1 = 0.0, iE2 = 0.0;
  if (use_elec) {
    for (unsigned short C=1; C<=Z1; C++) et1i += p1->dattrib[hybrid_pos+C]*C;
    for (unsigned short C=1; C<=Z2; C++) et2i += p2->dattrib[hybrid_pos+C]*C;
    for (size_t k=0; k<3; k++) {
      iE1 += p1->dattrib[use_elec+k] * p1->dattrib[use_elec+k];
      iE2 += p2->dattrib[use_elec+k] * p2->dattrib[use_elec+k];
    }
    iE1 *= 0.5*p1->mass * et1i * atomic_weights[0]/atomic_weights[Z1];
    iE2 *= 0.5*p2->mass * et2i * atomic_weights[0]/atomic_weights[Z2];
  }
  ...
  for (auto O : order) {
    ...
    if (ok) {
      ...
      // (e.g. C=1 is neutral).
      //
      unsigned short  Z1 = PP[cid]->k1.first;
      unsigned short  Z2 = PP[cid]->k2.first;

      Interact::pElem I1 = PP[cid]->swap ? std::get<2>(O) : std::get<1>(O);
      Interact::pElem I2 = PP[cid]->swap ? std::get<1>(O) : std::get<2>(O);
      
      unsigned short  P1 = I1.second;
      unsigned short  P2 = I2.second;
	...
	Etotl[id][ZZ] += Prob;
      ...
      //
      collTDPtr ctd1 = (*collD)[PP[cid]->k1];
      collTDPtr ctd2 = (*collD)[PP[cid]->k2];
	...
	if (not NOCOOL) ctd1->dv[id][2] += PP[1]->W2 * PE[1].second;
	if (scatter_check) {
	  double m1 = atomic_weights[Z1];
	  double m2 = atomic_weights[ 0];
	...
	if (not NOCOOL) ctd2->dv[id][2] += PP[2]->W2 * PE[2].second;
	if (scatter_check) {
  ...
  //------------------------------------------------------------
  // Sanity check energy loss accumulation.
  //
  // NB: For elastic interactions, delta E == 0.
  //
  if (PE[0].second < 0.0) {
  ...
  //
  // Update electron fractions
  //
  if (use_elec) {
    for (auto pp : PP)  pp->update();
  }
  ...
  if (use_elec) {
	...
	if (Z1 == Z2 or ALWAYS_APPLY) {
	  double DE1 = PE[0].first * p1->dattrib[use_cons];
	  double DE2 = PE[0].first * p2->dattrib[use_cons];
	...
	if (NOCOOL and KE_DEBUG) testCnt[id]++;
	...
	double delE = KE_initl_check - KE_final_check
	  - (deltaSum += KE.delta) - (delEsum += KE.delE) - (delEmis += KE.miss) - (delEdfr += KE.defer);
	...
	if (Z1 == Z2 or ALWAYS_APPLY) {
	  double DE1 = PE[1].first * p1->dattrib[use_cons];
	  double DE2 = PE[1].first * p2->dattrib[use_elec+3];
	  p1->dattrib[use_cons]   -= DE1;
	...
	if (KE_DEBUG) {
	  ...
	  ke2  *= 0.5*p2->mass * et2i * atomic_weights[0]/atomic_weights[Z2];
	  ke2f *= 0.5*p2->mass * et2f * atomic_weights[0]/atomic_weights[Z2];
	...
	if (KE_DEBUG) {
	  ...
	  ke1  *= 0.5*p1->mass * et1i * atomic_weights[0]/atomic_weights[Z1];
	  ke1f *= 0.5*p1->mass * et1f * atomic_weights[0]/atomic_weights[Z1];
  ...
  for (size_t k=0; k<3; k++) {
    if (use_cons>=0) {
      PP[k]->p1->dattrib[use_cons] -= PP[k]->E1.first;
      PP[k]->p2->dattrib[use_cons] -= PP[k]->E2.first;
    }
    if (use_elec>=0) {
      PP[k]->p1->dattrib[use_elec+3] -= PP[k]->E1.second;
      PP[k]->p2->dattrib[use_elec+3] -= PP[k]->E2.second;
    }
  }
  ...
  if (scatter_check) {
	...
	vi += vrel* vrel;
      ...
      Ediag[id][0] += PE[0].second;
      Ediag[id][1] += kE * PE[0].first;
      Ediag[id][2] += PE[0].first;
    }
	...
	vi += vrel* vrel;
      ...
      Ediag[id][0] += PE[1].second;
      Ediag[id][1] += kE * PE[1].first;
      Ediag[id][2] += PE[1].first;
    }
	...
	vi += vrel* vrel;
      ...
      Ediag[id][0] += PE[2].second;
      Ediag[id][1] += kE * PE[2].first;
      Ediag[id][2] += PE[2].first;
    }
  ...
  if (KE_DEBUG) {
    ...
    double delE = KEinitl - KEfinal - deltaSum - delEsum - delEmis - delEdfr;
 ...
 std::vector<double>* V1, std::vector<double>* V2, int id)
{
  ...
  // the velocity reduction factor
  //
  double q1 = pp->q * P;	// ACTIVE fraction relative to W1
  double q2 = P;		// ACTIVE fraction relative to W2
  double c1 = 1.0 - q1;		// INACTIVE fraction relative to W1
  double c2 = 1.0 - P;		// INACTIVE fraction relative to W1
  ...
  if (1) {
	...
	std::cout << "**GOOD scatter: delEt = "
		  ...
		  << "   v1 = " << std::setw(22) << KE1i << "   V1 = " << std::setw(22) << KE1f << std::setw(22) << Ii1 << std::setw(22) << Ie1
		  << "   v2 = " << std::setw(22) << KE2i << "   V2 = " << std::setw(22) << KE2f << std::setw(22) << Ii2 << std::setw(22) << Ie2
 ...
 unsigned iType, int id)
{
  ...
  if (equal and KE.delta>0.0) {
    if (pp->P == Pord::ion_ion) {
      pp->E1.first += KE.delta * 0.5;
      pp->E2.first += KE.delta * 0.5;
    } else {
      if (swap) {
	pp->E1.second += KE.delta * 0.5;
	pp->E2.first  += KE.delta * 0.5;
      } else {
	pp->E1.first  += KE.delta * 0.5;
	pp->E2.second += KE.delta * 0.5;
      }
    }
  }
  ...
  if (not equal) {
    if (TRACE_ELEC) {
      if (swap) {
	pp->E1.second += KE.delta * TRACE_FRAC;
	pp->E2.first  += KE.delta * (1.0 - TRACE_FRAC);
      } else {
	pp->E1.first  += KE.delta * (1.0 - TRACE_FRAC);
	pp->E2.second += KE.delta * TRACE_FRAC;
      }
    }
  }
...
void CollideIon::updateEnergyHybrid(PordPtr pp, KE_& KE)
{
  ...
  double rKEi=0.0, tKEi=0.0;	// Total pre collision KE
  double rKEf=0.0, tKEf=0.0;	// Total post collision KE

  if (pp->P == Pord::ion_electron) {
    if (pp->swap) {
      rKEi = pp->Einit [1]              + pp->Einit [2];
      rKEf = pp->Efinal[1]              + pp->Efinal[2];
      tKEi = pp->Einit [1] * pp->Eta[0] + pp->Einit [2];
      tKEf = pp->Efinal[1] * pp->Eta[2] + pp->Efinal[2];
      //                ^            ^
      //                |            |
      // Particle 1/electron---------+
    } else {
      rKEi = pp->Einit [0] + pp->Einit [3] ;
      rKEf = pp->Efinal[0] + pp->Efinal[3] ;
      tKEi = pp->Einit [0] + pp->Einit [3] * pp->Eta[1];
      tKEf = pp->Efinal[0] + pp->Efinal[3] * pp->Eta[3];
      //                                ^            ^
      //                                |            |
      // Particle 2/electron------------+------------+
    }
  ...
  //
  double testE = (tKEi - tKEf) - (rKEi - rKEf);
  ...
  if (scatter_check and aType==Hybrid) {
	...
	std::cout << std::setw(14) << dSum
      ...
      std::cout << "           " << std::setw(14) << "Tot"
		...
		<< "Total dE = "
		<< std::setw(14) << Etots[0]
		<< std::setw(14) << Etots[0]/Etots[2] << std::endl
		<< "Total KE = "
		<< std::setw(14) << Etots[1]
		<< std::setw(14) << Etots[1]/Etots[2] << std::endl
		<< "Total #  = "
...
CollideIon::Pord::Pord(CollideIon* c, Particle *P1, Particle *P2, double WW1, double WW2,
		       pType p, double T) :
  ...
  thresh(T), swap(false), wght(false), E1({0, 0}), E2({0, 0})
{
  ...
  //
  eta1 = eta2 = 0.0;
  for (unsigned short C=1; C<=Z1; C++) eta1 += p1->dattrib[caller->hybrid_pos+C]*C;
  for (unsigned short C=1; C<=Z2; C++) eta2 += p2->dattrib[caller->hybrid_pos+C]*C;
...
std::array<double, 4> CollideIon::Pord::compE()
{
  std::array<double, 4> ret;	// zero initialized
  ...
  if (wght) {
    ...
    W2 = w2 * eta2;
				...
				// Compute KE
  for (size_t k=0; k<3; k++) {
    ret[0] += p1->vel[k] * p1->vel[k];
    ret[2] += p2->vel[k] * p2->vel[k];
    if (caller->use_elec>=0) {
      ret[1] += p1->dattrib[caller->use_elec+k] * p1->dattrib[caller->use_elec+k];
      ret[3] += p2->dattrib[caller->use_elec+k] * p2->dattrib[caller->use_elec+k];
    }
  }
  ret[0] *= 0.5 * w1 * m10;
  ret[1] *= 0.5 * w1 * atomic_weights[0]/m10;
  ret[2] *= 0.5 * w2 * m20;
  ret[3] *= 0.5 * w2 * atomic_weights[0]/m20;
