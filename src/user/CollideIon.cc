...
const int N_Z = 2;		//Number of Ion elements

const int ZList[N_Z] = {1, 2}; //Array of the different Z's for the elements

const double abund[N_Z] = {0.97, 0.03}; //Harcoded abundance array
  ...
  Collide(force, diameter, Nth)
{
  ...
  NUM = 0;
  cout << "Creating CollideIon instance" << endl;
  csections = std::vector<sKey2Dmap> (nthrds);
  
  cout << "\tCreating ion list" << endl;
  for(int i = 0; i < N_Z; i++) {
  ...
  cout << "No cooling is set to: " << NO_COOL << endl;
}
...
{
  //cout << "Initializing cell in CollideIon " << endl;
  if (firstFlag == 0) {
    double cross = M_PI*diam*diam;
    for (sKey2Umap::iterator it1 = nsel.begin(); it1 != nsel.end(); it1++) 
      {
	speciesKey i1 = it1->first;
	for (sKeyUmap::iterator it2 = it1->second.begin(); it2 != it1->second.end(); it2++) 
	  {
	    speciesKey i2 = it2->first;
	    csections[id][i1][i2] = 0.25*(i1.first*i1.first + i2.first*i2.first)*cross;
	  }
      }
			  ...
			  double *cr, int id)
{
  ...
  //double fac1 = (atomic_weights[p1->Z]*amu)/(me);
  //double fac2 = (atomic_weights[p2->Z]*amu)/(me);

  double m1, m2;
  m1 = atomic_weights[p1->Z]*amu;
  m2 = atomic_weights[p2->Z]*amu;
  ...
  double kEI = 0.5*muI*vr*vr;
  //factor in the energy spread to all the particles
  kEI *= 2.0/(2.0+ne1+ne2);
  double pe = sqrt(2.*me*kEI);
  //cout << "Electron velocity = " << pe/me << endl;
  double kEe;
  kEe = kEI;
  kEe = kEe*6.241509e11;
  //double kEeV = kE*6.241509e11;

  double dE = kE*TolV*TolV;
  double remE = (kE - dE);
  double delE=0.0;

  //cout << "total Ke = " << kE << " with cr = " << (*cr) << " dE = " << dE << " remE = " << remE << endl;
  ...
  ***/
  ...
  if (ne2 > 0 and p1->C <= p1->Z) {
	  ...
	  CE1 = IonList[p1->Z][p1->C].collExciteCross(ch, pe, E_therm_1, me);
	   //if (ne2*CE1.back().first != 0) {cout << "\tCE1: " << CE1.back().first*ne2 << endl;}
	  //if (CE1.back().first != 0) cout << "CE = " << ne2*CE1.back().first << " E = " << E_therm_1 << " Z = " << p1->Z << " C = " << p1->C <<"\t";
  ...
  if (p1->C < (p1->Z + 1) and ne2 > 0) {
	DI1 = IonList[p1->Z][p1->C].directIonCross(ch, kEe);
        //if (DI1 != 0) { cout << "\tDI1 = " << DI1 << endl;CECIflag=1;}
  ...
  if (p2->C > 1 and ne1 > 0) {
	  ...
	  //if (ff2 != 0) cout << "FF2: " << ff2 << " kE = " << kEe << endl;
	  dCross.push_back(ne1*ff2);
	  sum21 += ff2*ne1;
	  inter.push_back(6);
  }
  //cout << "SEVENTH\t (Z,C) = " << p2->Z << "," << p2->C << ")" << "\t";
  std::vector< std::pair<double, double > > CE2;
  if(ne1 > 0 and p2->C <= p2->Z) {
          assert(E_therm_2 != 0);
	  CE2 = IonList[p2->Z][p2->C].collExciteCross(ch, pe, E_therm_2, me);
	  //if (ne1*CE2.back().first != 0) {cout << "\tCE2: " << CE2.back().first*ne1 << endl;}
	  //if (CE2.back().first != 0) cout << "CE = " << ne1*CE2.back().first << "\t";
  ...
  if (p2->C < (p2->Z + 1) and ne1 > 0) {
	DI2 = IonList[p2->Z][p2->C].directIonCross(ch, kEe);
        //if (DI2 != 0) {cout << "\tDI2 = " << DI2 << endl;CECIflag=1;}
  ...
  if (p2->C > 1 and ne1 > 0) {
	...
	//cout << "RR2 = " << ne1*RE2.back() << " kE = " << kEe << endl;
	sum21 += RE2.back()*ne1;
	inter.push_back(9);
	outflag = 0;
  } 
  ...
  // Now that the interactions have been calculated, create the normalized cross section list to pick the interaction
  std::vector<double> TotalCross;
  double tCross;
  tCross = 0;
  int si = 0;
  for(int i = 0; i < dCross.size(); i++) {
	tCross += dCross[i];
  ...
  if (tCross != 0) {
	  ...
	  std::vector <double>::iterator max = max_element(TotalCross.begin(), TotalCross.end());
	  for (int i = 0; i < TotalCross.size(); i++) {
	    normed.push_back(TotalCross[i]/tCross);
	  }

	  // Now that the normed probablility cross section vector is created, pull a random number
	  double ran;
	  ran = (*unit)();
	  int index;
  ...
  if(delE > kE) cout << "delE > KE!! Interaction = " << interFlag << " kEe = " << kEe << " kE = " << kE << " delE = " << delEeV << endl;
  //if (delE != 0.) cout << "delE = " << delE << "kE = " << kE << " remE = " << remE << endl;
  ...
  //add to the total cross section the "ballistic" minimum bohr radius for the cross section. THe first part is then the "interaction" section, and the second is the elastic collision
  csections[id][j2][j1] = (sum12*1e-14)/(UserTreeDSMC::Lunit*UserTreeDSMC::Lunit)+0.25*(p1->Z*p1->Z+p2->Z*p2->Z)*M_PI*diam*diam; //convert to cm^2 first, then system units
  csections[id][j1][j2] = (sum21*1e-14)/(UserTreeDSMC::Lunit*UserTreeDSMC::Lunit)+0.25*(p1->Z*p1->Z+p2->Z*p2->Z)*M_PI*diam*diam;
  ...
  if (remE >= delE) {
    ...
    dv.first++; dv.second=vi-(*cr);
    //if (interFlag == 1 or interFlag == 6)
    	//cout << "Before: " << vi << " After: " << (*cr) << " dE = " << delE << endl;
    ret = 0;			// No error
    ...
    cout << "************remE < delE!!!!" << " RemE = " << remE << " delE = " << delE << " interaction: " << interFlag << endl;
    lostSoFar[id] += remE;
    decolT[id]    += remE - delE;
    (*cr) *= TolV;
...
void CollideIon::printCollSummary() {
	if(myid == 0) {
			cout << "Collision debugging:" << endl;
			cout << "id:ffN:ffE:CEN:CEE:CIN:CIE:RRN:RRE:dv" << endl;
			cout << myid << ff_d.first << " : " << ff_d.second << " : " << CE_d.first << " : " << CE_d.second << " : " << CI_d.first << " : " << CI_d.second << " : " << RR_d.first << " : " << RR_d.second << " : " << dv.second/dv.first << endl;
		}
	else { cout << myid << ff_d.first << " : " << ff_d.second << " : " << CE_d.first << " : " << CE_d.second << " : " << CI_d.first << " : " << CI_d.second << " : " << RR_d.first << " : " << RR_d.second << " : " << dv.second/dv.first<< endl; }
      if(myid == 0) {
			cout << "Energy debugging:" << endl;
			cout << "id:av:min:max:over10" << endl;
			cout << myid << " : "  << eV_av/eV_N << " : " << eV_min << " : " << eV_max << " : " << eV_10 << endl;
		}
	else { cout << myid << " :" << eV_av/eV_N  << " : "<< eV_min <<" : "<< eV_max << endl; }
}
...
void CollideIon::resetColls() {
	...
	eV_N = 0.; eV_av = 0.; eV_max = 0.; eV_min = 99999.; eV_10++;
}
...
void * CollideIon::timestep_thread(void * arg)
{
  ...
  for (unsigned j=0; j<cellist[id].size(); j++ ) {
    ...
    for (it1=c->count.begin(); it1!=c->count.end(); it1++) {
	  ...
	  //cout << "N = " << N << " dens = " << densM[i2] << endl;
	  crossIJ[i1][i2] = max(crossIJ[i1][i2], (M_PI*0.25*diam*diam*(i1.first*i1.first + i2.first*i2.first)));
	  if (i2>=i1) {
	    crossM[i1] += N*densM[i2]*(min(crossIJ[i1][i2], (M_PI*0.25*diam*diam*(i1.first*i1.first + i2.first*i2.first))));
	    //cout << i1.first << " " << i1.second << " " << i2.first << " " << i2.second << " " <<crossIJ[i1][i2] << "\n";
	  } else
	    crossM[i1] += N*densM[i2]*(min(crossIJ[i2][i1], (M_PI*0.25*diam*diam*(i1.first*i1.first + i2.first*i2.first))));
	    //cout << i2.first << " " << i2.second << " " << i1.first << " " << i1.second << " " << crossIJ[i2][i1] << "\n";
        }
      
      //cout << "crossM = " << crossM[i1] << " densM = " << densM[i1] << "Fn: " << (*Fn)[i1] << endl;
    ...
    //cout << meanLambda << "\t" << meanLambda*UserTreeDSMC::Lunit << endl;
...
/*void CollideIon::list_sizes_proc(ostream* out)
{
