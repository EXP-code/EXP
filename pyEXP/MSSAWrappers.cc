...
void MSSAtoolkitClasses(py::module &m) {
  ...
  m.doc() =
    ...
    "Detrending\n"
    "----------\n"
    "expMSSA automatically detrends the data channels before the SVD analysis\n"
    "of the trajectory matrix.  Detrending is a common step in MSSA analyses to\n"
    "reduce the bias of feature detection by the measurement process itself.\n"
    "The default method subtracts the mean and scales by the variance in each\n"
    "channel. This detrending strategy promotes the detection of weak signals\n"
    "and is the most common method for MSSA analyses.  However, you can also\n"
    "choose to detrend according to the total variance in all channels after\n"
    "mean subtraction ('totVar') or by the total power in all channels ('totPow').\n"
    "These two methods will give more weight to channels with the largest variance\n"
    "or power, which may be appropriate in some cases.  In addition, the 'noMean'\n"
    "option will prevent the mean from being  subtracted from the data channels\n"
    "while using the 'totPow' detrending method.  For example, this allows one\n"
    "to select signals according to total gravitational energy.  A future\n"
    "enhancement will allow the user to specify a custom detrending scheme.\n\n"
    "Grouping\n"
    "--------\n"
