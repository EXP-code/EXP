...
main(int argc, char **argv)
{
  ...
  while (1)
	...
	case 0:			// Long options
	  ...
	  if (!optname.compare("smooth")) SMOOTH = atoi(optarg);
  ...
  if (n_particlesH) expandh = new SphericalSL(LMAX, NMAX, SCSPH);
  ...
  if (myid==0 && basis) {
    ...
    if (n_particlesH) {
      ...
      for (int i=0; i<nstr; i++) {
        ...
        out[i].write(&nout, sizeof(int));
        out[i].write(&nout, sizeof(int));
        out[i].write(&(f=-rmax), sizeof(float));
        out[i].write(&(f= rmax), sizeof(float));
        out[i].write(&(f=-rmax), sizeof(float));
        out[i].write(&(f= rmax), sizeof(float));
      }
      ...
      for (int j=0; j<nout; j++) {
        ...
        for (int i=0; i<nout; i++) {
          ...
          out[0].write(&(f=dens), sizeof(float));
          out[1].write(&(f=potl), sizeof(float));
          out[2].write(&(f=potr), sizeof(float));
          out[3].write(&(f=pott), sizeof(float));
          out[4].write(&(f=potp), sizeof(float));
        }
    ...
    if (n_particlesD) {
	  ...
	  if (x<0.0)
	    ...
	    d = expandd->accumulated_dens_eval(x, y, 0.0);
          ...
          out[3].write(&(f=fz), sizeof(float));
          out[4].write(&(f=fp), sizeof(float));
        }
