	 ...
	 string& filename, int DF1, int DIVERGE, double DIVERGE_RFAC)
{
  ...
  halo = new SphericalModelTable(String(filename.c_str()), 
...
{
  const double tol = 1.0e-8;
  double rmin = disk->get_min_radius();
...
table_disk(vector<Particle>& part)
{
  ...
  for (int i=ibeg[myid]; i<iend[myid]; i++) {
	...
	disktableP[i][j][k] = max<double>(workA[NDZ1-1] - workA[k*NDZF], 
      ...
      if (i==ibeg[myid] && j==0) {
	ostrstream ofile;
	ofile << "intgr_disk_P.d" << myid << '\0';
	ofstream dout(ofile.str());
	for (int k=0; k<NDZ1; k++) 
...
void DiskHalo::write_record(ofstream &out, Particle &p)
{
  ...
  for (int k=0; k<3; k++)
    out << setw(24) << setprecision(15) << p.pos[k];
  
  for (int k=0; k<3; k++)
    out << setw(24) << setprecision(15) << p.vel[k];
			  ...
			  vector<Particle>& hpart, vector<Particle>& dpart)
{
  ...
  // Get displacements
  MPI_Aint	disp[4];
  MPI_Get_address(&buf.comp,	&disp[0]);
  MPI_Get_address(&buf.mass,	&disp[1]);
  MPI_Get_address(&buf.pos,	&disp[2]);
  MPI_Get_address(&buf.vel,	&disp[3]);
  ...
  if (myid==0) {
	...
	MPI_Recv(&buf, 1, Particletype, n, 11, MPI_COMM_WORLD, &status);
	write_record(fou_halo, buf);
      }
      
      MPI_Recv(&imany, 1, MPI_INT, n, 10, MPI_COMM_WORLD, &status);
      for (int i=0; i<imany; i++) {
	MPI_Recv(&buf, 1, Particletype, n, 11, MPI_COMM_WORLD, &status);
	write_record(fou_disk, buf);
      }
	...
	for (int i=0; i<l; i++) {
	  pbuf = &hpart[i];
	  MPI_Send(pbuf, 1, Particletype, 0, 11, MPI_COMM_WORLD);
	}
...
double DiskHalo::virial_ratio(vector<Particle>& hpart, vector<Particle>& dpart)
{
...
double DiskHalo::virial_ratio(const char *filename)
{
  ifstream in(filename);
  if (!in) {
    cout << "virial_ratio: can't open " << filename << endl;
    return 0.0;
  }
  ...
  in.getline(linebuffer, linesize);
  istrstream ins(linebuffer);
  ins >> expected;
  ...
  cout << endl
       ...
       <<"  KE_halo  = " << KE_halo << endl
       <<"  KE_disk  = " << KE_disk << endl << endl
       <<"  PE_halo  = " << PE_halo << endl
       <<"  PE_disk  = " << PE_disk << endl << endl;
