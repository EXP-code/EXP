...
float_mat::float_mat(const size_t rows,const size_t cols,const double defval)
    ...
    if ((rows < 1) || (cols < 1)) {
        ...
        msg << "cannot build matrix with " << rows << " rows and " << cols
               << "columns";
...
#if 0
    if (piv_elem < tol) {
        throw std::runtime_error("partial_pivot(): Zero pivot encountered.");
#endif
...
static int lu_factorize(float_mat &A, int_vect &idx, double tol=TINY_FLOAT)
{
    ...
    if ((A.nr_rows() == 0) || (A.nr_rows() != A.nr_cols())) {
        throw std::runtime_error("lu_factorize(): cannot handle empty "
    ...
    for (i = 0; i < A.nr_rows(); ++i) {
        ...
        if (maxval == 0.0) {
            throw std::runtime_error("lu_factorize(): zero pivot found.");
        }
        scale[i] = 1.0 / maxval;
    }
...
float_mat operator *(const float_mat &a, const float_mat &b)
{
    ...
    if (a.nr_cols() != b.nr_rows()) {
        throw std::runtime_error("incompatible matrices in multiplication");
    }
...
float_vect sg_smooth(const float_vect &v, const int width, const int deg)
{
    ...
    if ((width < 1) || (deg < 0) || (v.size() < (2 * width + 2))) {
        throw std::runtime_error("sgsmooth: parameter error.");
    }
                         ...
                         const int deg, const double h)
{
    ...
    if ((width < 1) || (deg < 1) || (v.size() < (2 * width + 2))) {
        throw std::runtime_error("sgsderiv: parameter error");
    }
